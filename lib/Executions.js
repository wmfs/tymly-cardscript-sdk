...
module.exports = class Executions {
  ...
  /**
   * Count the entries IndexedDB where localState is specified
   * @param {string} localState - Local State to filter.
   */
  async getLocalStateCount (localState) {
    const data = await this.db.executions.toArray()
    let count = 0
    for (const d of data) {
      if (d.localState === localState) {
        count++
      }
    }
    return count
