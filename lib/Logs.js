...
module.exports = class Logs {
  ...
  async loadLogs (options) {
    ...
    const wanted = LOG_LEVELS.slice(0, LOG_LEVELS.indexOf(logLevel) + 1)
    if (logLevel === 'ALL') {
      payload = data
      ...
      for (const d of data) {
        wanted.forEach((level) => {
          if (d.message.includes(level)) payload.push(d)
        })
      }
    }
 ...
 */
function formatLog ({ level, code, title, body }) {
  return {
    message: body ? `[${new Date()}] [${level}] ${code}: ${title}\n${body}` : `[${new Date()}] [${level}] ${code}: ${title}`,
    date: new Date()
  }
}
