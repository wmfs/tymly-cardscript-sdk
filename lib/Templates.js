...
class Templates {
  ...
  async load (id) {
    if (id) {
      ...
      const cs = await this.processCardscript(template)
      const rawTemplate = {
        uiName: id,
        ...template,
        ...cs
  ...
  async processCardscript (card) {
    ...
    if (validatorOutput.elementsValid) {
      ...
      const { template } = await quasarConverter(
        card,
        {
          imageSourceTemplate: `url(${this.imageSourceTemplate})`,
          checkStateMachinePermissions: this.checkStateMachinePermissions.bind(this)
        }
      )
      r.quasarTemplate = template
  ...
  async checkStateMachinePermissions (stateMachineName) {
    const roleId = this.auth.sub
    const token = this.auth.token

    const res = await this.executions.execute({
      stateMachineName: 'tymly_rbacCheckAuth_1_0',
      input: { stateMachineName, roleId },
      token
    })

    return res.ctx.allowed
  }
}
