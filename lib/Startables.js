module.exports = class Startables {
  ...
  async persistFromUserQuery (userQuery) {
    ...
    if (startablesToAdd) {
      for (const startable of Object.values(startablesToAdd)) {
        startable._modified = new Date()
        await this.db.startables.put(startable)
      }
    }

    if (startablesToRemove) {
      for (const startable of startablesToRemove) {
        await this.db.startables.delete(startable)
      }
    }
