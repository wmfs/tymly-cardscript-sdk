...
module.exports = class TymlyClient {
  ...
  async requestUserQuery (requestKey) {
    ...
    if (requestKey) {
      ...
      for (const key of USER_QUERY_KEYS) {
        ...
        stopwatch.addTime(`Persisted ${key} from user query to indexed db`)
      }
      await this.loadFromDb()
  ...
  async getUserQuery (stopwatch) {
    ...
    const clientManifest = {
      cardNames: store.state.app.templates.reduce((acc, curr) => {
        const name = `${curr.namespace}_${curr.name}_${curr.version.replace(/\./g, '_')}`
        acc[name] = curr.shasum
        return acc
      }, {}),
      startable: store.state.app.startables.reduce((acc, curr) => {
        acc[curr.name] = curr.shasum
        return acc
      }, {}),
      todos: store.state.app.todos.reduce((acc, curr) => {
