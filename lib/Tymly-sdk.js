...
module.exports = class TymlyClient {
  ...
  async loadFromDb () {
    await Promise.all(USER_QUERY_KEYS.map(key => this[key].load()))
  }

  async persistFromUserQuery (userQuery) {
    await Promise.all(USER_QUERY_KEYS.map(key => this[key].persistFromUserQuery(userQuery)))
  }
  ...
  async requestUserQuery (requestKey) {
    ...
    if (requestKey) {
      ...
      stopwatch.addTime(`Persisted ${requestKey} from user query to indexed db`)

      await this[requestKey].load()
      stopwatch.addTime(`Loaded ${requestKey} from indexed db to vuex store`)
    } else {
      await this.persistFromUserQuery(userQuery)
      stopwatch.addTime('Persisted all from user query to indexed db')

      await this.loadFromDb()
      stopwatch.addTime('Loaded all from indexed db to vuex store')
    }
