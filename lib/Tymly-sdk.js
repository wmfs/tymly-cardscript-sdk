...
module.exports = class TymlyClient {
   ...
   */
  async init () {
    console.log('Starting to initialise SDK...')
  ...
  async getUserQuery (stopwatch) {
    const { appStore: store, auth } = this.options
    const { token } = auth
    ...
    for (const t of templates) {
      ...
      clientManifest.cardNames[name] = t.shasum
    }
    for (const s of startables) {
      clientManifest.startable[s.name] = s.shasum
    }
    for (const t of todos) {
      const copyTodo = JSON.parse(JSON.stringify(t))
      delete copyTodo.shasum
      clientManifest.todos[copyTodo.id] = shasum(JSON.stringify(copyTodo))
    }
    for (const c of categories) {
      clientManifest.categoryNames.push(c.category)
    }
    // todo clientManifest.teams e.g. ['Birmingham (Red watch)', 'Another team']
    /*
    const clientManifest = {
   ...
   */
  async destroy () {
    this.options.appStore.$reset()
    this.options.trackerStore.$reset()
    this.haveRunUserQuery = false
    this.sdkHasInitialized = false
