...
module.exports = class TymlyClient {
   ...
   */
  async requestUserQuery (options = {}) {
    const { requestKey } = options

    const token = this.options.auth.token

    const versionRes = await this.executions.execute({ stateMachineName: 'tymly_getVersions_1_0', input: {}, token })
    const apiVersionShasum = versionRes.ctx.versionShasum
    // const storedApiVersionShasum = await this.db.auth.get('api')
    const storedApiVersionShasum = { id: 'api', value: '123' } // todo: remove - for testing

    if (storedApiVersionShasum) {
      if (storedApiVersionShasum.value !== apiVersionShasum) {
        console.log('Invalidating Cache')
        await this.db.auth.put({ id: 'api', value: apiVersionShasum })
      }
    } else {
      console.log('Storing API Version Shasum')
      await this.db.auth.put({ id: 'api', value: apiVersionShasum })
    }
