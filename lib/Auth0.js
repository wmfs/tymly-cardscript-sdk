...
module.exports = class Auth0 {
   ...
   * Clear tokens/profile info etc.
   */
  async setFromLocalStorage () {
    this.triedToGetAToken = true
    const token = (await this.db.auth.get('token'))
    if (token && token.value) {
      console.log('FOUND TOKEN IN LOCAL STORAGE')
      this.token = token.value
      this.accessToken = (await this.db.auth.get('accessToken')).value
      this.expiresIn = (await this.db.auth.get('expiresIn')).value
      this.idToken = (await this.db.auth.get('idToken')).value
      this.tokenType = (await this.db.auth.get('tokenType')).value
      this.scope = (await this.db.auth.get('scope')).value
      this.name = (await this.db.auth.get('name')).value
      this.picture = (await this.db.auth.get('picture')).value
      this.email = (await this.db.auth.get('email')).value
      this.timestamp = (await this.db.auth.get('timestamp')).value
      return true
    } else {
      return false
    }
