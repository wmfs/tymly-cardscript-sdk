module.exports = class Auth0 {
  ...
  /**
   * Acknowledges a tour is finished by this user, and stores their auth0 sub in DB
   * @example
   */
  async tourCompleted () {
    const { allowed, completed } = await this.checkTour()
    if (allowed) {
      completed.push(this.sub)
    }
    this.db.auth.put({ id: 'tourComplete', value: completed })
  }

  /**
   * Checks local DB to see if user should see tour
   * @example
   */
  async checkTour () {
